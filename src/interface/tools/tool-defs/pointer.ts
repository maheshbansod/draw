...
export class PointerTool implements Tool {
	...
	const setUpDragAbility = () => {
		...
		const alreadySelectedSets = elementsStore.getSelectedLineSegmentSets();
		if (!alreadySelectedSets[0]) {
		    const selected = elementsStore.selectLineSegmentAt(x, y);
		    if (selected) {
			elementsStore.setSelectedLineSegmentSet(selected);
		    }
		}
		const newlySelectedSets = elementsStore.getSelectedLineSegmentSets();
		if (newlySelectedSets[0]) {
		    startMousePos = {x, y};
		    initialSegmentSetPosition = newlySelectedSets[0].firstPoint();
		    cachedCanvasData = this.ctx.getImageData(0, 0, this.ctx.canvas.width, this.ctx.canvas.height);
		...
		endMousePos = {x, y};
		const currentFirstPoint = selectedSegmentSets[0].firstPoint();
		const mouseDelta = {
		    x: x - startMousePos.x,
		    y: y - startMousePos.y
		};
